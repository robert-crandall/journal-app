...
export function getClassesByType() {
	return {
		Combat: CLASS_LIBRARY.filter((cls) =>
			['Fighter', 'Warrior', 'Paladin', 'Barbarian', 'Ranger'].includes(cls.class_name)
		),
		Mystic: CLASS_LIBRARY.filter((cls) =>
			['Wizard', 'Sorcerer', 'Warlock', 'Druid', 'Cleric'].includes(cls.class_name)
		),
		Support: CLASS_LIBRARY.filter((cls) =>
			['Bard', 'Healer', 'Support', 'Guardian', 'Protector'].includes(cls.class_name)
		),
		Stealth: CLASS_LIBRARY.filter((cls) =>
			['Rogue', 'Assassin', 'Scout', 'Thief', 'Spy'].includes(cls.class_name)
		),
		Other: CLASS_LIBRARY.filter((cls) =>
			['Monk', 'Artificer', 'Scholar', 'Merchant', 'Wanderer'].includes(cls.class_name)
		)
	};
}
