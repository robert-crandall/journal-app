<script lang="ts">
	...
	onMount(() => {
		...
		async function loadData() {
			try {
				...
				adhocTasks = (adhocTasksData.adhocTasks || []).filter(
					(adhocTask: any) => !completedAdhocTaskIdsToday.includes(adhocTask.id)
				);
	...
	async function submitAdhocTaskExecution(adhocTaskId: string) {
		try {
			...
			const [statsData, tasksData] = await Promise.all([statsApi.getAll(), tasksApi.getAll()]);
			stats = statsData.slice(0, 4);
