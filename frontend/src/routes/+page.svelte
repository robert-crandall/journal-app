<script lang="ts">
	...
	// Using actual seeded data from backend
	const DEMO_USER_ID = 'a80f659e-a987-4190-9aef-efe16c7061c0'; // From seed script output
	const DEMO_CHARACTER_ID = '50ebe305-182f-49b8-bdb3-1612971e1970'; // From seed script output
	...
	async function loadDashboard() {
		try {
			...
			console.log('Loading character data for character:', DEMO_CHARACTER_ID);

			// Get today's tasks from dashboard endpoint
			const tasksResponse = await api.dashboard.$get({
				query: { 
					userId: DEMO_USER_ID,
					status: 'pending',
					limit: '10'
			...
			// Get character data
			const characterResponse = await api.characters[DEMO_CHARACTER_ID].dashboard.$get({
				query: { userId: DEMO_USER_ID }
			});

			console.log('Character response status:', characterResponse.status);
			if (!characterResponse.ok) {
				throw new Error(`Failed to load character data: ${characterResponse.status} ${characterResponse.statusText}`);
			}
			...
			// Combine data
			dashboardData = {
				user: { id: DEMO_USER_ID, email: 'demo@example.com', name: 'Demo User' } as any,
				character: characterData.data?.character || null,
				stats: characterData.data?.stats || null,
				todaysTasks: tasksData.data?.tasks || [],
				xpProgress: {
					currentLevel: Math.floor(characterData.data?.overallProgression?.averageLevel || 1),
					currentXp: characterData.data?.overallProgression?.totalXp || 0,
					xpToNextLevel: 500, // Calculate based on next level
					totalXp: characterData.data?.overallProgression?.totalXp || 0
				},
		...
		<div class="bg-card p-4 rounded-lg border border-border">
			<div class="flex items-center justify-between">
				<div>
					...
					<p class="text-2xl font-bold text-foreground">{Math.floor(dashboardData.xpProgress.currentLevel)}</p>
				</div>
