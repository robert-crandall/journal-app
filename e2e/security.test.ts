...
test.describe('Security Tests', () => {
  ...
  test('validates email format properly', async ({ page }) => {
    ...
    const invalidEmails = ['invalid-email', '@journal.com', 'user@', 'user@.com', 'user..name@journal.com'];
    ...
    for (const invalidEmail of invalidEmails) {
      ...
      await expect(page.locator('.alert-error')).toContainText('Please enter a valid email address');
  ...
  test('prevents SQL injection attempts', async ({ page }) => {
    const sqlInjectionPayloads = ["'; DROP TABLE users; --", "admin' OR '1'='1", "' UNION SELECT * FROM users --"];
  ...
  test('handles concurrent login attempts gracefully', async ({ browser }) => {
    ...
    await Promise.all([page2.fill('input[name="email"]', userCredentials.email), page3.fill('input[name="email"]', userCredentials.email)]);

    await Promise.all([page2.fill('input[name="password"]', userCredentials.password), page3.fill('input[name="password"]', userCredentials.password)]);
