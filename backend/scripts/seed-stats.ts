...
// Export the seed function to be used by other scripts
export async function seed(database: typeof db) {
  try {
    logger.info('Seeding stat groups and templates...');
    
    // First, insert all the stat groups
    logger.info('Creating stat groups...');
    
    const existingGroups = await database.select().from(statGroups);
    if (existingGroups.length > 0) {
      logger.warn(`Found ${existingGroups.length} existing stat groups. Skipping group creation.`);
    } else {
      await database.insert(statGroups).values(groups);
      logger.info(`Created ${groups.length} stat groups successfully`);
    }

    // Now fetch all groups to get their IDs
    const allGroups = await database.select().from(statGroups);
    ...
    // Check for existing templates
    const existingTemplates = await database.select().from(statTemplates);
    if (existingTemplates.length > 0) {
      logger.warn(`Found ${existingTemplates.length} existing stat templates. Skipping template creation.`);
      return;
    }
    ...
    // Insert all templates
    logger.info('Creating stat templates...');
    await database.insert(statTemplates).values(templates);
    logger.info(`Created ${templates.length} stat templates successfully`);
    
    logger.info('Stats seeding completed successfully.');
  } catch (error) {
    logger.error('Error in seed function:', error);
    throw error;
  }
}

// Run the seeding function if script is called directly
if (import.meta.main) {
  await createStatTemplates();
}
