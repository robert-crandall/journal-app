...
export async function callGptApi(options: GptOptions): Promise<GptResponse> {
  ...
  if (process.env.NODE_ENV === 'test' || process.env.OPENAI_API_KEY === 'sk-test-key-for-testing-only') {
    ...
    } else if (content.includes('Generate metadata') || content.includes('Analyze this journal conversation') || content.includes('analyze this journal entry')) {
      // Return properly formatted JSON for journal analysis
      mockResponse = JSON.stringify({
        summary: 'In this journal session, the user shared their thoughts and feelings in a meaningful conversation. The discussion touched on personal experiences and provided space for reflection.',
