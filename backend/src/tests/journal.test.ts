...
describe('Journal API', () => {
  ...
  describe('GET /api/journal/:id', () => {
    ...
    beforeEach(async () => {
      ...
      await db.insert(journalToneTags).values([{ journalId: testJournalId, tag: 'happy' }]);

      await db.insert(journalStatTags).values([{ journalId: testJournalId, statId, xpAmount: 15 }]);
    });
  ...
  describe('PUT /api/journal/:id', () => {
    ...
    it('should update a journal entry', async () => {
      ...
      // Verify in database
      const journalEntries = await db.select().from(journals).where(eq(journals.id, testJournalId));
    ...
    it('should not allow updating a finalized journal', async () => {
      ...
      await db.update(journals).set({ isFinalized: true }).where(eq(journals.id, testJournalId));
  ...
  describe('DELETE /api/journal/:id', () => {
    ...
    beforeEach(async () => {
      ...
      // Add some tags
      await db.insert(journalContentTags).values([{ journalId: testJournalId, tag: 'delete-test' }]);
    });
    ...
    it('should delete a journal entry and its tags', async () => {
      ...
      const contentTags = await db.select().from(journalContentTags).where(eq(journalContentTags.journalId, testJournalId));
  ...
  describe('POST /api/journal/finalize', () => {
    ...
    it.skip('should finalize and analyze a journal entry', async () => {
      ...
      const toneTags = await db.select().from(journalToneTags).where(eq(journalToneTags.journalId, testJournalId));

      expect(toneTags.length).toBe(2); // focused, tired

      const statTagsResults = await db.select().from(journalStatTags).where(eq(journalStatTags.journalId, testJournalId));
