...
describe('Journal Day Rating', () => {
  ...
  it('should infer a day rating when finishing a journal without a user-provided rating', async () => {
    ...
    // Start reflection
    const startReflectionResponse = await request(app)
      .post(`/api/journals/${testDate}/start-reflection`)
      .set('Authorization', `Bearer ${authToken}`)
      .send({});
    ...
    // Add a chat message
    await request(app)
      .post(`/api/journals/${testDate}/chat`)
      .set('Authorization', `Bearer ${authToken}`)
      .send({
    ...
    const finishResponse = await request(app)
      .post(`/api/journals/${testDate}/finish`)
      .set('Authorization', `Bearer ${authToken}`)
      .send({});
