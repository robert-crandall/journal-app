...
const app = new Hono()
  ...
  .get('/', jwtAuth, zValidator('query', getUserAttributesQuerySchema as any), async (c) => {
    try {
  ...
  .post('/deduplicate', jwtAuth, zValidator('query', deduplicationQuerySchema as any), async (c) => {
    try {
      const userId = getUserId(c);
      const query = c.req.valid('query');
  ...
  .post('/', jwtAuth, zValidator('json', createUserAttributeSchema as any), async (c) => {
    try {
      const userId = getUserId(c);
  ...
  .post('/bulk', jwtAuth, zValidator('json', bulkCreateUserAttributesSchema as any), async (c) => {
    try {
      const userId = getUserId(c);
      const data = c.req.valid('json');
  ...
  .put('/:id', jwtAuth, zValidator('json', updateUserAttributeSchema as any), async (c) => {
    try {
      const userId = getUserId(c);
      const attributeId = c.req.param('id');
      const data = c.req.valid('json');
