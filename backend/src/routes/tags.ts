...
const app = new Hono()
  ...
  // Create a new tag
  .post('/', jwtAuth, zodValidatorWithErrorHandler('json', createTagSchema as any), async (c) => {
    try {
      const userId = getUserId(c);
  ...
  // Batch create tags
  .post('/batch', jwtAuth, zodValidatorWithErrorHandler('json', batchCreateTagsSchema as any), async (c) => {
    try {
      const userId = getUserId(c);
  ...
  // Update a tag
  .put('/:id', jwtAuth, zodValidatorWithErrorHandler('json', updateTagSchema as any), async (c) => {
    try {
      const userId = getUserId(c);
      const tagId = c.req.param('id');
