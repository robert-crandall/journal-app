...
app.post('/generate', jwtAuth, zValidator('json', generateMetricsSchema as any), async (c) => {
  try {
...
app.get('/', jwtAuth, zValidator('query', listMetricSummariesSchema as any), async (c) => {
  try {
    const userId = getUserId(c);
    const {
      type,
...
app.get('/:id', jwtAuth, zValidator('param', metricSummaryIdSchema as any), async (c) => {
  try {
    const userId = getUserId(c);
...
app.delete('/:id', jwtAuth, zValidator('param', metricSummaryIdSchema as any), async (c) => {
  try {
    const userId = getUserId(c);
    const { id } = c.req.valid('param');
