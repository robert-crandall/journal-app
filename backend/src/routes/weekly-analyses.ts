...
const app = new Hono()
  ...
  .get('/', jwtAuth, zodValidatorWithErrorHandler('query', listWeeklyAnalysesSchema as any), async (c) => {
    try {
  ...
  .get('/:id', jwtAuth, zodValidatorWithErrorHandler('param', weeklyAnalysisIdSchema as any), async (c) => {
    try {
      const userId = getUserId(c);
  ...
  .post('/', jwtAuth, zodValidatorWithErrorHandler('json', createWeeklyAnalysisSchema as any), async (c) => {
    try {
      const userId = getUserId(c);
  ...
  .post('/generate', jwtAuth, zodValidatorWithErrorHandler('json', generateWeeklyAnalysisSchema as any), async (c) => {
    try {
      const userId = getUserId(c);
