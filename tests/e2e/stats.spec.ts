...
test.describe('Stats Management', () => {
	...
	test('should create a custom stat successfully', async ({ page }) => {
		...
		await page.fill(
			'input[placeholder="e.g., Programming, Fitness, Creativity"]',
			'Programming Skills'
		);
		await page.fill(
			'textarea[placeholder*="Describe what this stat represents"]',
			'Track my progress in learning new programming languages and frameworks'
		);

		// Fill example activities
		await page.fill(
			'input[placeholder*="Complete a coding challenge"]',
	...
	test('should grant XP to a stat', async ({ page }) => {
		...
		await page.fill(
			'textarea[placeholder*="Completed a 30-minute workout"]',
			'Completed a 45-minute strength training session'
		);
		...
		await expect(
			page.locator('text=Completed a 45-minute strength training session')
		).toBeVisible();
	...
	test('should handle level up when enough XP is earned', async ({ page }) => {
		...
		await page.fill(
			'textarea[placeholder*="Completed a 30-minute workout"]',
			'Cooked a complex three-course meal'
		);
		...
		await page.fill(
			'textarea[placeholder*="Completed a 30-minute workout"]',
			'Prepared an elaborate dinner party'
		);
		await page.click('button[type="submit"]:has-text("Grant 100 XP")');
		...
		await page.fill(
			'textarea[placeholder*="Completed a 30-minute workout"]',
			'Mastered a new cooking technique'
		);
		await page.click('button[type="submit"]:has-text("Grant 100 XP")');
	...
	test('should edit an existing stat', async ({ page }) => {
		...
		await page
			.locator('#description')
			.fill('Track creative writing projects and daily writing practice');
