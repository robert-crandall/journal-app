...
test.describe('Family Management', () => {
  ...
  test('should display XP history for family member', async ({ page }) => {
    await page.goto('/family');

    // Create a family member
    const uniqueName = `XP Test ${Date.now()}`;
    ...
    await page.fill('input[placeholder="Their name"]', uniqueName);
    await page.fill('input[placeholder="How they\'re related to you"]', 'Child');
    await page.click('button[type="submit"]:has-text("Create Family Member")');

    // Go to the family member's detail page
    await page.goto('/family');
    await page.click(`text=${uniqueName}`);

    // Check the Connection Level/XP is shown
    await expect(page.locator('text=Connection Level').first()).toBeVisible();
    await expect(page.locator('text=Connection XP').first()).toBeVisible();

    // Initially, might not have any XP history yet
    if (await page.locator('text=No interactions recorded yet').isVisible()) {
      // Add a task/interaction to generate XP
      await page.goto('/family');
      await page.click(`text=${uniqueName}`);

      // Find the Edit button and navigate to edit page
      await page.click('button:has-text("Edit")');

      // Add notes and update to trigger an interaction
      await page.fill('textarea[name="notes"]', 'Adding notes to generate an interaction');
    ...
    // Now we should be able to see the XP card
    await expect(page.locator('.stat:has-text("Connection XP")').first()).toBeVisible();

    // The Connection Level stat card should be visible
    await expect(page.locator('.stat:has-text("Connection Level")').first()).toBeVisible();
  });
});
