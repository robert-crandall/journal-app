...
/**
 * Helper function to cleanup journal entries
 */
export async function cleanupJournalEntries(page: Page): Promise<void> {
  try {
    // Navigate to journal page
    await page.goto('/journal');
    await page.waitForLoadState('networkidle');

    // Check if there are any entries and delete them
    // This is a simple cleanup - in a real app you might want more sophisticated cleanup
    let attempts = 0;
    const maxAttempts = 10;
    ...
    while (attempts < maxAttempts) {
      try {
        ...
        const entryCard = page.locator('.card').first();
        const isVisible = await entryCard.isVisible();

        if (!isVisible) {
          break; // No more entries
        }

        // Click on entry to go to details
        await entryCard.click();
        await page.waitForLoadState('networkidle');

        // Look for delete button (if it exists)
        // Note: Based on current implementation, there might not be a delete feature yet
    ...
    // If cleanup fails entirely, continue with tests
    console.warn('Journal cleanup failed:', error);
  }
}
