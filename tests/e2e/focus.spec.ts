...
async function cleanupFocuses(page) {
  try {
    ...
    for (let i = 0; i < Math.min(count, 10); i++) {  // Limit to 10 iterations for safety
      ...
      if (await deleteButton.isVisible()) {
        ...
        const confirmDeleteButton = page
          .locator('.card:has(.card-title:has-text("Confirm Delete")) button.btn-error:has-text("Delete")');
