...
import { getFamilyMemberById, updateFamilyMember } from '$lib/server/family.js';
...
export const load: PageServerLoad = async ({ locals, params }) => {
  ...
  const familyMember = await getFamilyMemberById(params.id, locals.user.id);
  if (!familyMember) {
...
export const actions: Actions = {
  default: async ({ request, locals, params }) => {
    ...
    try {
      await updateFamilyMember(params.id, locals.user.id, {
        name: name.trim(),
