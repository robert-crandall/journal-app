...
export async function getUserStats(userId: string): Promise<StatWithActivities[]> {
	...
	for (const stat of userStats) {
		...
		const activities = await db
			.select()
			.from(statActivities)
			.where(eq(statActivities.statId, updatedStat.id))
			.orderBy(statActivities.description);
 ...
 */
export async function addStatActivity(
	statId: string,
	description: string,
	suggestedXp: number = 10
): Promise<StatActivity> {
 ...
 */
export async function updateStatActivity(
	activityId: string,
	updates: Partial<Pick<StatActivity, 'description' | 'suggestedXp'>>
): Promise<StatActivity> {
